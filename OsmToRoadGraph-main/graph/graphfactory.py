...
def _add_nodes(g: Graph, id_mapper: Dict[int, int], nodes: Dict[int, OSMNode]) -> None:
    for n in nodes.values():
        g.add_node(Vertex(n.osm_id, id_mapper[n.osm_id], data=VertexData(n.lat, n.lon)))
...
def _add_edges(g: Graph, id_mapper: Dict[int, int], ways: List[OSMWay]) -> None:
    ...
    for w in ways:
        for i in range(len(w.nodes) - 1):
            ...
            edge = Edge(w.nodes[i], w.nodes[i + 1], s_id, t_id, w.forward, w.backward, data=data)
...
def build_graph_from_vertices_edges(vertices: List[Vertex], edges: List[Edge]) -> Graph:
    ...
    for v in vertices:
        g.add_node(Vertex(v.id, id_mapper[v.id], v.data))
