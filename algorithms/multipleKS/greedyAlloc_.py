...
@timeout(60)
def greedyAlloc(cloudlets, vms, detectedCloudletsPerUser, withQuadtree, sortOption):
    ...
    D = utils.calcEfficiency(normalVms, 'sum')
    ...
    for d in D:
        currentUser = d[0]
        c = chooseCloudlet(currentUser, cloudlets, cloudletsOccupation, detectedCloudletsPerUser, withQuadtree, sortOption)
        if c is not None:
...
# this method used to be called 'firstFit' before the tests with the bin packing algorithms
def chooseCloudlet(user, cloudlets, cloudletsOccupation, detectedCloudletsPerUser, withQuadtree, sortOption):
    if withQuadtree:
        sortedCloudlets = utils.sortCloudletsByType(detectedCloudletsPerUser[user.uId], user, cloudletsOccupation, sortOption)
        for c in sortedCloudlets:
...
def pricing(winners, users, detectedCloudletsPerUser, cloudlets, withQuadtree, sortOption):
    ...
    for w in winners:
        ...
        for d in D:
            ...
            if c is not None:
                utils.allocate(currentUser, cloudletsOccupation[c.cId])
            if chooseCloudlet(w[0], cloudlets, cloudletsOccupation, detectedCloudletsPerUser, withQuadtree, sortOption) is None:
